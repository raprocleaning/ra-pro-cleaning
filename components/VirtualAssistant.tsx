...
function getBotReply(userText: string): Message {
  ...
  if (t.includes('deep')) {
    return {
      from: 'bot',
      text: "Our Deep Cleaning is a thorough top-to-bottom clean — every surface, corner, cabinet interior, baseboards, and appliance. Starting at $700. Perfect for first-time clients or seasonal resets.",
  ...
  if (t.includes('standard') || t.includes('regular') || t.includes('recurring')) {
    return {
      from: 'bot',
      text: "Our Standard Cleaning is perfect for recurring maintenance — keeping your home consistently clean between deep cleans. Starting at $600. We can set up weekly, bi-weekly, or monthly visits!",
      options: ['How do I book?', 'How much does it cost?', 'What else do you offer?'],
    }
  }
...
export default function VirtualAssistant() {
  ...
  return (
    <>
      {/* WhatsApp Button */}
      <a
        href="https://wa.me/17206778799"
        target="_blank"
        rel="noopener noreferrer"
        aria-label="Chat on WhatsApp"
        className="fixed bottom-24 right-6 z-50 w-12 h-12 bg-[#25D366] hover:bg-[#1ebe5d] text-white shadow-lg flex items-center justify-center transition-all duration-300 rounded-full"
      >
